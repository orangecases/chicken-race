...
async function loadUserData(user) {
    ...
    try {
        // [?섏젙] 'set-then-listen' ?⑦꽩?쇰줈 沅뚰븳 ?ㅻ쪟瑜?理쒖쥌 ?닿껐?⑸땲??
        // 1. 癒쇱? 臾몄꽌瑜??앹꽦/?낅뜲?댄듃?섏뿬 議댁옱瑜?蹂댁옣?⑸땲??
        //    - Firestore 蹂댁븞 洹쒖튃??'create'媛 ?덉슜?섏뼱 ?덉뼱???⑸땲??
        //    - merge:true ?듭뀡?쇰줈 湲곗〈 ?좎????곗씠?곕뒗 ??뼱?곗? ?딆뒿?덈떎.
        ...
        const finalNickname = (extractedNickname || '?대쫫?놁쓬') + providerSuffix;
        ...
        // set({ merge: true })??臾몄꽌媛 ?놁쑝硫??앹꽦?섍퀬, ?덉쑝硫??꾨뱶瑜?蹂묓빀?⑸땲??
        // ?좉퇋 ?좎???寃쎌슦 initialUserData濡?臾몄꽌媛 ?앹꽦?⑸땲??
        // 湲곗〈 ?좎???寃쎌슦, ??set? ?꾨Т寃껊룄 ??뼱?곗? ?딆?留?臾몄꽌??議댁옱瑜?蹂댁옣?섎뒗 ??븷???⑸땲??
        ...
        // 2. ?댁젣 臾몄꽌媛 ?뺤떎??議댁옱?섎?濡? ?ㅼ떆媛?由ъ뒪?덈? ?덉쟾?섍쾶 遺李⑺빀?덈떎.
        let initialLoadComplete = false;
        unsubscribeUserData = userRef.onSnapshot((snapshot) => {
            // ???쒖젏?먮뒗 snapshot.exists媛 ??긽 true?ъ빞 ?⑸땲??
            if (!snapshot.exists) {
        ...
        console.error("???좎? ?곗씠??珥덇린 濡쒕뵫/?앹꽦 ?ㅽ뙣:", error);
        alert("?좎? ?뺣낫瑜?遺덈윭?ㅻ뒗 以??ㅻ쪟媛 諛쒖깮?덉뒿?덈떎.");
    }
}
